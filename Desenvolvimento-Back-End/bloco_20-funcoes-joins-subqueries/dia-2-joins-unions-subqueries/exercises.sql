...
AND EXISTS (
	SELECT * FROM Pixar.Theater
    WHERE id = mov.theater_id
);

-- Exercício 11: Utilizando o SELF JOIN , selecione os títulos e duração dos filmes que possuem o mesmo diretor.
SELECT
	a.title, a.length_minutes, b.title, b.length_minutes
FROM Pixar.Movies a, Pixar.Movies b
...
WHERE mov.id IN (
	SELECT movie_id FROM Pixar.BoxOffice box
    WHERE (box.domestic_sales + box.international_sales) > 500000000 
    )
