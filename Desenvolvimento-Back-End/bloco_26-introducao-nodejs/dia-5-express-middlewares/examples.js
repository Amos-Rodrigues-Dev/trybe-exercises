...
function validateName(req, res, next) {
  const { name } = req.body;
  if (!name || name === '') return res.status(400).json({ message: 'Invalid data! (name)'}); // 1
...
function validatePrice(req,res, next) {
  ...
  if(typeof price !== "number" || price === '' || price < 0) return res.status(400).json({ message: 'Invalid data! (price)' }); 
