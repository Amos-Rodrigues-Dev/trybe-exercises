import React from 'react';
// importe o connect do pacote 'react-redux'
import PropTypes from 'prop-types';
// importe a ação oriunda do thunk
import { getInfoGamesThunk } from '../actions'
import './SearchForm.css';
import { connect } from 'react-redux';

class SearchForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      inputText: '',
      characterSearched: '',
      isDisabled: false,
    };
    this.handleChange = this.handleChange.bind(this);
    this.submitName = this.submitName.bind(this);
  }

handleChange(e) {
  const { inputText } = this.state;
  this.setState({
    inputText: e.target.value,
    characterSearched: '',
    isDisabled: inputText.length >= 4,
  })
}

submitName(e) {
  e.preventDefault();
  const { inputText } = this.state;
  //desestruture a ação do thunk como propriedade aqui
  const { getInfoGame } = this.props;

  this.setState({
    inputText: '',
    characterSearched: inputText,
  })
  // insira a action a ser despachada para o thunk
  getInfoGame(inputText);
}

render() {
  const { inputText, isDisabled } = this.state;

  return (
    <div>
      <form onSubmit={this.submitName}>
        <h1>Type a character below:</h1>
        <input onChange={this.handleChange} 
        placeholder="Enter Character"
        value={inputText}
        />
        <div className="buttonSection">
          <button disabled={ !isDisabled } className="submitButton" type="submit" >Search!</button>
        </div>
      </form>
    </div>
  )
}
};

// mapeie as ações despachadas como propriedade do componente
const mapDispatchToProps = (dispatch) => ({
  getInfoGame: (character) => dispatch(getInfoGamesThunk(character)),
});

// conecte as ações despachadas ao redux
export default connect(null, mapDispatchToProps)(SearchForm);

//faça as proptypes da ação oriunda do thunk
SearchForm.propTypes = {
  getInfoGame: PropTypes.func.isRequired,
}
